...
VM_PROJECT_PATH="${VM_PROJECT_PATH:-/Users/omaralzhrani/defthon}"
...
if orb_ok 'ip link show dummy0'; then
    if orb_ok "ip link show dummy0 | grep -q 'state UP'"; then
...
if orb_ok "[ -r /Users/omaralzhrani/defthon ]"; then
    ok "Mac home auto-mounted in VM"
else
    fail "VM cannot read /Users/omaralzhrani/defthon"
