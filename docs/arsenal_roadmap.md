...
| 3 | 9 — systemd + container | ✅ | stdin-exit bug fixed (headless mode: no-TTY → datapath stays up, controlled via WS + SIGINT; main-side so it exits cleanly). systemd unit + `install.sh` + `/etc/hakam/hakam.env` — **validated in VM**: `systemctl start`→active/armed/headless/WS, `systemctl stop`→clean detach. Container (`packaging/docker/` multi-stage Dockerfile + entrypoint + run.sh + .dockerignore) — **build-tested + validated** (Docker 29 in VM): image builds incl. bpf-linker (166 MB); `docker run --privileged --network host` arms XDP+TC+BPF-LSM headless + serves telemetry; a SQLi was dropped by the container datapath (`BLOCK`/`XDP_DROP`); `docker stop` exited 0 with hooks detached. Core-only; UI documented as optional. |
| 3 | 10 — Driver-mode XDP / real NIC | ✅ | Driver-mode (native) XDP validated — `--mode drv` attaches natively (confirmed on a veth: `mode=Driver`, `prog/xdp id` present), not SKB-only. Bench rig (`bench/`) + in-VM CSV results measure CPU%/pps/latency deltas and are honest that absolute pps is environment-bound. **Owner-accepted residual:** a bare-metal real-NIC line-rate pass — physically needs hardware (VM tops ~5–10k pps; `bench/README` documents this). README perf *table* is #12. |
| 3 | 11 — Small PCAP replay corpus | ✅ | `corpus/pcaps/` — 4 real captures (SQLi, XSS, LFI×2) + `scripts/replay-corpus.sh` (tcpreplay → asserts each BLOCK on the WS feed). Validated in VM against a fresh node: all 4 blocked with expected source+category. Honest note in `corpus/README`: `cmd-injection` blocks via the `/etc/passwd` LFI token — Hakam has no dedicated command-injection family. |
