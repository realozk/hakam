...
//!   * No reordering. Without seq numbers (added by Phase 2 #7) we append
//!     in arrival order. Out-of-order delivery can break a split signature
//!     across two segments — documented as a known gap.
//!   * No retransmit dedupe. Same reason. A retransmit appends a duplicate
//!     of an earlier payload to the buffer; for substring matching this is
//!     harmless (the match still fires) but it eats buffer space faster.

use std::collections::HashMap;

use hakam_common::PayloadEvent;
...
impl Reassembler {
    ...
    pub fn ingest(&mut self, key: FlowKey, payload: &[u8], now_ns: u64) -> Option<&[u8]> {
        ...
        let state = self.flows.get_mut(&key).expect("just inserted");
...
mod tests {
    ...
    fn single_segment_is_returned_intact() {
        ...
        let view = r.ingest(k(40000), b"hello", 0).unwrap();
    ...
    fn two_segments_concatenate() {
        let mut r = Reassembler::with_defaults();
        let _ = r.ingest(k(40000), b"GET /?x=UNI", 100);
        let view = r.ingest(k(40000), b"ON SELECT 1 HTTP/1.1", 200).unwrap();
        assert!(
    ...
    fn distinct_flows_dont_bleed() {
        let mut r = Reassembler::with_defaults();
        let _ = r.ingest(k(40000), b"AAAA", 0);
        let _ = r.ingest(k(40001), b"BBBB", 0);
        let view_a: Vec<u8> = r.ingest(k(40000), b"AAAA", 0).unwrap().to_vec();
        let view_b: Vec<u8> = r.ingest(k(40001), b"BBBB", 0).unwrap().to_vec();
    ...
    fn forget_drops_state() {
        let mut r = Reassembler::with_defaults();
        let _ = r.ingest(k(40000), b"xxx", 0);
    ...
    fn gc_evicts_stale_flows() {
        ...
        let _ = r.ingest(k(40000), b"old", 0);
        let _ = r.ingest(k(40001), b"new", 1_500_000_000);
    ...
    fn buffer_cap_clamps_growth() {
        ...
        let _ = r.ingest(k(40000), b"AAAAAAAA", 0); // exactly at cap
        let view = r.ingest(k(40000), b"BBBB", 0).unwrap();
    ...
    fn flow_cap_refuses_new_flows() {
        ...
        let _ = r.ingest(k(40000), b"a", 0);
        let _ = r.ingest(k(40001), b"b", 0);
        let result = r.ingest(k(40002), b"c", 0);
