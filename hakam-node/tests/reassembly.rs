...
//! matcher against the reassembled view.
...
fn split_union_select_caught_after_segment_two() {
    ...
    let v1: Vec<u8> = r.ingest(k, b"GET /?q=UNI", 100).unwrap().to_vec();
    assert!(
    ...
    let v2: Vec<u8> = r.ingest(k, b"ON SELECT 1 HTTP/1.1", 200).unwrap().to_vec();
...
fn split_across_three_segments() {
    ...
    let _ = r.ingest(k, b"GET /?q=", 0);
    let _ = r.ingest(k, b"UN", 0);
    let v: Vec<u8> = r.ingest(k, b"ION SELECT 1 HTTP/1.1", 0).unwrap().to_vec();
...
fn forget_after_match_lets_same_flow_be_reinspected() {
    ...
    let v1: Vec<u8> = r
        .ingest(k, b"GET /?q=UNION SELECT 1 HTTP/1.1", 0)
        .unwrap()
        .to_vec();
